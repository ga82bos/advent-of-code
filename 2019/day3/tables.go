...
func main() {
	...
	// [i][j] int [row][col]int
	coordSystem := make([][]string, size)
	for i := range coordSystem {
		coordSystem[i] = make([]string, size)
	}

	coordSystem[size/2][size/2] = "O"
	coordSystem = drawPath(line2, 1, coordSystem, dir2Vec)
	coordSystem = drawPath(line1, 2, coordSystem, dir2Vec)
	//printCoordSys(coordSystem)
}

func printCoordSys(coords [][]string) {
	intersections := ""
	for i := range coords {
		for j := range coords[i] {
			...
			if s == "X" {
				intersections += fmt.Sprintf("(%d,%d)=%d ", i-size/2, j-size/2, abs(i-size/2)+abs(j-size/2))
			}
			fmt.Printf(s)
		}
		fmt.Println()
...
func drawPath(line string, lineID int, coordSys [][]string, dir2Vec map[string]Coord) [][]string {
	coords := strings.Split(line, ",")
	lID := strconv.Itoa(lineID)
	...
	for _, vec := range coords {
		direction, length := strToVec(vec)
		v := dir2Vec[direction]
		for i := 0; i < length; i++ {
			row += v.Y
			col += v.X
			coordSys = mark(row, col, lID, pathLen, coordSys)
			pathLen++
		}
	}
	return coordSys
...
func mark(row, col int, marker string, pathLen int, coordSys [][]string) [][]string {
	// if row index wouldn't fit in slice, resize coordSys
	if row >= cap(coordSys) {
		newCoordSys := make([][]string, row*2) // double size
		for i := range newCoordSys {
			newCoordSys[i] = make([]string, cap(coordSys[0]))
			if i < (len(coordSys) - 1) {
				copy(newCoordSys[i], coordSys[i])
			}
		}
		copy(newCoordSys, coordSys)
		coordSys = newCoordSys
	}

	// if col wouldnt fit, resize all columns in order to keep an MxN array
	if col >= cap(coordSys[row]) {
		for i := range coordSys {
			for range coordSys[i] {
				newRow := make([]string, col*2)
				copy(newRow, coordSys[i])
				coordSys[i] = newRow
			}
		}
	}
	isSecondLine := marker == "2"
	curr := coordSys[row][col]
	newChar := ""
	switch curr {
	case "": // empty => write current PathLen
		if !isSecondLine {
			newChar = strconv.Itoa(pathLen)
		}
	default: // else: print
		if isSecondLine {
			pathLenLine1, _ := strconv.Atoi(curr)
			fmt.Printf("Intersection at pathLen(%s)=%d pathLen(1)=%d (%d,%d)=%d TOTAL=%d\n", marker, pathLen, pathLenLine1, row-size/2, col-size/2, abs(row-size/2)+abs(col-size/2), pathLenLine1+pathLen)
		}
	}

	coordSys[row][col] = newChar
	return coordSys
}

func strToVec(vec string) (string, int) {
	direction := string(vec[0])
	switch direction {
	case "R", "L", "U", "D": // correct
	default:
		log.Fatal(fmt.Sprintf("invalid coord: %s", vec))
	}

	l := vec[1:]
	length, err := strconv.Atoi(l)
	if err != nil {
		log.Fatal(fmt.Sprintf("invalid length: %s", l))
	}
